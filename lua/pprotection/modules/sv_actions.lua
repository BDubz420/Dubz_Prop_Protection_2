...
function DPP2:GetPlayerEntities(target)
    local sid = target:SteamID64()
    local out = {}

    for _, ent in ipairs(ents.GetAll()) do
        if IsValid(ent) and not ent:IsPlayer() and ent.DPP2GetOwnerSID64 and ent:DPP2GetOwnerSID64() == sid then
            out[#out + 1] = ent
        end
    end
...
function DPP2:GhostEntity(ent, enabled)
...
function DPP2:RunEntityAction(admin, target, action)
    ...
    for _, ent in ipairs(self:GetPlayerEntities(target)) do
        ...
        elseif action == "highlight" then
            ent:SetNWBool("DPP2Highlighted", true)
            timer.Simple(12, function()
                if IsValid(ent) then ent:SetNWBool("DPP2Highlighted", false) end
            end)
        end
    end
...
function DPP2:GlobalAction(admin, action)
